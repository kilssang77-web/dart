...
def auto_register_from_inpo21c(
    ...
    company_profile의 company_name / biz_reg_no를 기준으로 inpo21c_participants에서
    우리 회사 참여 건을 전수 조회하여, 아직 bid_journal에 없는 건을 모두 등록한다.
    낙찰/패찰 결과, 사정율, 낙찰자 투찰률, 순위, 참여자수 자동 기입.
    """
    from sqlalchemy import text
    from app.models import BidJournal, Bid
    from app.journal_service import _sync_actual_outcome
    ...
    if not profile:
        ...
        raise HTTPException(400, "company_profile 미등록. 회사 프로파일을 먼저 설정하세요.")
    ...
    # 이미 등록된 공고번호
    existing_anos = {r[0] for r in db.execute(text(
    ...
    rows = db.execute(text(f"""
        SELECT
            ...
            (SELECT ip2.bid_rate FROM inpo21c_participants ip2
             WHERE ip2.inpo21c_bid_id = ip.inpo21c_bid_id AND ip2.is_winner = true LIMIT 1) AS winner_bid_rate,
            (SELECT COUNT(*) FROM inpo21c_participants ip3
             WHERE ip3.inpo21c_bid_id = ip.inpo21c_bid_id) AS total_participants
    ...
    for r in rows:
        ...
        try:
            # bid_id 조회 또는 stub 생성
            bid = None
            if ann_no:
                bid = db.query(Bid).filter(Bid.announcement_no == ann_no).first()
            if not bid:
                # agency_id 조회: inpo21c agency_name → agencies 테이블
                fallback_agency_id = 8533  # 미상
                if agency_name:
                    ag_row = db.execute(text(
                        "SELECT id FROM agencies WHERE name = :n LIMIT 1"
                    ), {"n": agency_name}).fetchone()
                    if ag_row:
                        fallback_agency_id = ag_row[0]
                bid = Bid(
            ...
            rate_gap = None
            if winner_rate and submitted_rate:
                rate_gap = round(winner_rate - submitted_rate, 6)

            # pred_srate_center ML 예측
            pred_srate_center = None
            srate_error = None
            if bid.base_amount:
                try:
                    from app.ml.assessment import load_srate_stats, predict_srate
                    ...
                    if actual_srate and pred_srate_center:
