...
async def list_performance(
    ...
    min_prob = 0.0
    try:
        raw = await redis.get("telegram:config")
        if raw:
            min_prob = float(json.loads(raw).get("min_prob", 0.0))
    except Exception:
        pass
    ...
    if min_prob > 0:         conds.append(f"rec.success_prob>={p(min_prob)}")
    ...
    """
    sql_total = f"""
        SELECT COUNT(*) FROM recommendation_performance rp
        JOIN recommendations rec ON rec.id = rp.rec_id
        {where}
    """
...
async def performance_summary(
    ...
    cache_key = f"cache:perf_summary:{days}:{min_prob}"
    try:
    ...
    sql = """
        SELECT
            COUNT(*)                                              AS total,
            COUNT(*) FILTER (WHERE rp.tracking_complete)         AS completed,
            COUNT(*) FILTER (WHERE rp.is_success = TRUE)         AS success,
            COUNT(*) FILTER (WHERE rp.is_success = FALSE)        AS fail,
            ROUND(AVG(rp.r_1d)::NUMERIC,2)                       AS avg_r_1d,
            ROUND(AVG(rp.r_3d)::NUMERIC,2)                       AS avg_r_3d,
            ROUND(AVG(rp.r_5d)::NUMERIC,2)                       AS avg_r_5d,
            ROUND(AVG(rp.r_10d)::NUMERIC,2)                      AS avg_r_10d,
            ROUND(AVG(rp.max_return)::NUMERIC,2)                 AS avg_max_return,
            ROUND(
              100.0 * COUNT(*) FILTER (WHERE rp.is_success=TRUE) /
              NULLIF(COUNT(*) FILTER (WHERE rp.tracking_complete),0)
            ,1)                                                   AS success_rate,
            COUNT(*) FILTER (WHERE rp.hit_target)                AS hit_target_cnt,
            COUNT(*) FILTER (WHERE rp.hit_stop)                  AS hit_stop_cnt
        FROM recommendation_performance rp
        JOIN recommendations rec ON rec.id = rp.rec_id
        WHERE rp.signal_time >= NOW() - ($1 * INTERVAL '1 day')
          AND COALESCE(rec.success_prob, 0) >= $2
    """
    by_event_sql = """
        SELECT rp.event_type,
               COUNT(*) AS cnt,
               ROUND(100.0 * COUNT(*) FILTER (WHERE rp.is_success) / NULLIF(COUNT(*) FILTER (WHERE rp.tracking_complete),0),1) AS win_rate,
               ROUND(AVG(rp.r_5d)::NUMERIC,2) AS avg_r5d
        FROM recommendation_performance rp
        JOIN recommendations rec ON rec.id = rp.rec_id
        WHERE rp.signal_time >= NOW() - ($1 * INTERVAL '1 day')
          AND rp.event_type IS NOT NULL
          AND COALESCE(rec.success_prob, 0) >= $2
        GROUP BY rp.event_type ORDER BY cnt DESC
    """
    async with request.app.state.db.acquire() as conn:
        row      = await conn.fetchrow(sql, days, min_prob)
        ev_rows  = await conn.fetch(by_event_sql, days, min_prob)
    result = {
        **dict(row),
