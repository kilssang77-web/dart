...
from routers import stocks, features, recommendations, disclosures, backtest, themes, market, disclosure_filters, ml, news, watchlist, settings, notifications, tracking, admin, ranking, screener, trader, auth as auth_router, history
...
async def lifespan(app: FastAPI):
    ...
    if app.state.db is None:
        logger.warning("DB 미연결 — 최소 기능으로 앱 시작")
        yield
...
app.include_router(history.router,        prefix="/api/v1/chart",             tags=["history"])
