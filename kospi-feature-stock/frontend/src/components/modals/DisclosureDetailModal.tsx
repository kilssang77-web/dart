...
import { X, Building2, FileText, Tag, TrendingUp, TrendingDown, Calendar, DollarSign, Lightbulb } from 'lucide-react'
...
function buildDisclosureNarrative(d: Disclosure): string {
  ...
  const scoreStr = d.sentiment_score != null
    ...
    : ''
  const amtStr   = d.amount ? `공시와 관련된 금액은 <strong>${fmt.amount(d.amount)}</strong>입니다. ` : ''
...
export function DisclosureDetailModal({ disclosure: d, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div
        className="relative w-full max-w-xl bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-2xl overflow-hidden"
      ...
      >
        ...
        {/* 지표 4박스 */}
        <div className="grid grid-cols-2 gap-3 px-6 py-5 border-b border-[var(--border)]/60">
          ...
          <div className="bg-[var(--bg)] rounded-xl p-4">
            ...
            <div className="text-base font-semibold text-[var(--fg)] tabular">
          ...
          <div className="bg-[var(--bg)] rounded-xl p-4">
            ...
            <div className={clsx('text-xl font-bold tabular', pctColor(d.post_1d_change))}>
        ...
        {d.keywords && d.keywords.length > 0 && (
          <div className="px-6 py-4 border-b border-[var(--border)]/60">
            <div className="text-xs font-semibold text-[var(--muted)] mb-2.5 uppercase tracking-wider">키워드</div>
