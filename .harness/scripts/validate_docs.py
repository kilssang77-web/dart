...
def validate(stage: str = "mvp") -> dict:
    ...
    # 각 문서 내용 검사 — 현재 stage의 필수 문서만 검사한다
    for path in sorted(DOCS_DIR.glob("*.md")):
        if path.stem in required_docs or path.stem in HARNESS_INTERNAL_DOCS:
            all_issues.extend(check_file(path, stage))

    # 디렉토리화된 문서 내용 검사 (INDEX.md + 하위 파일)
    for sub_dir in sorted(DOCS_DIR.iterdir()):
        if sub_dir.is_dir() and sub_dir.name in required_docs:
